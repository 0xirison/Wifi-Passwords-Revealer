...
==========================  """)
        ...
        for line in show_wifi_command.stdout.decode('latin-1').splitlines():
            if "All" in line.replace(" ",""):
                delim = ": "
                wifi_list.append(line[line.index(delim) + len(delim):])

        for wifi_details in wifi_list:
            show_wifi_password = subprocess.run('netsh wlan show profile '+ wifi_details + " key=clear", shell=False, stdout=subprocess.PIPE)
            for line in show_wifi_password.stdout.decode('latin-1').splitlines():
                if "Key Content" in line.replace("\r\n", "").strip():
                    delim = ": "
                    wifi_dict[wifi_details] = line[line.index(delim) + len(delim):]
                else:
                    continue
        file= open(os.path.normpath(os.path.expanduser("~/Desktop")).replace("\\",'\\\\')+ "\\Wifi_Passwords-" +str(datetime.datetime.now()).replace(":", "-").replace(" ","-").split(".")[0] +".txt","w+")
